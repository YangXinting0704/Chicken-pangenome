...
# 用法: Rscript PanBaRT_build_from_gtf.R <input_gtf> <prefix> [chimeric_tolerance=0.05]
# 说明: input_gtf 是 minimap2 --> genePred-->gtf 的结果，把 GsRTD 的 transcripts mapping 到 linear pan 的 gtf
...
# ---------------------------------------------------------------------
# Step 1: 多外显子与单外显子转录本处理（作者的 step1）
# ---------------------------------------------------------------------
...
if(length(idx) > 0){
  ...
  # 按 pair1 分组、对每个 transcript 单独 reduce() 并标注 transcript_id，然后合并
